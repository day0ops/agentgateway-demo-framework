...
export function ModelCostsPage() {
  ...
  const { data: costs, loading, refresh } = useApi(
    useCallback(() => modelCostsApi.list(), [])
  );

  const createMutation = useMutation(modelCostsApi.create);
  const updateMutation = useMutation(
    useCallback((modelId: string, data: CreateModelCostRequest) =>
      modelCostsApi.update(modelId, data), [])
  );
  ...
  return (
    <Container>
      <PageHeader
        title="Model Costs"
        description="Configure pricing for LLM models"
      >
              ...
              {costs.map((cost) => (
                <TableRow key={cost.model_id}>
                  ...
                  <TableCell align="right">
                    {formatCost(cost.input_cost_per_million)}
                  </TableCell>
                  <TableCell align="right">
                    {formatCost(cost.output_cost_per_million)}
                  </TableCell>
                  <TableCell>{cost.model_pattern || '—'}</TableCell>
                  <TableCell align="right">
                    <ActionButtons>
                      <ActionButton onClick={() => handleEdit(cost)}>
                        Edit
                      </ActionButton>
                      <ActionButton onClick={() => setDeleteTarget(cost)}>
                        Delete
                      </ActionButton>
                    </ActionButtons>
                  </TableCell>
