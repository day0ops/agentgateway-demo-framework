...
export function BudgetDetailPage() {
  ...
  const { data: budget, loading: budgetLoading, refresh: refreshBudget } = useApi(
    useCallback(() => (id ? budgetsApi.get(id) : Promise.reject('No ID')), [id])
  );

  const { data: usageRecords, loading: usageLoading, refresh: refreshUsage } = useApi(
    useCallback(() => (id ? budgetsApi.getUsage(id, undefined, 100) : Promise.reject('No ID')), [id])
  );

  const resetMutation = useMutation(budgetsApi.reset);
  const updateMutation = useMutation(
    useCallback((budgetId: string, data: { enabled: boolean; version?: number }) =>
  ...
  const usagePercent = budget.budget_amount_usd > 0
    ? (budget.current_usage_usd / budget.budget_amount_usd) * 100
    : 0;
