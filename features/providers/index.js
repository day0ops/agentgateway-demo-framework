...
export class ProvidersFeature extends Feature {
  ...
  getResourceName(baseName) {
    const gatewayName = this.deployGatewayRef?.name || 'agentgateway';
    if (gatewayName === 'agentgateway') {
      return baseName;
    }
  ...
  async cleanup() {
    ...
    if (this.spinner?.isSpinning) {
      this.spinner.setText('Cleaning up provider AgentgatewayBackends...');
    }
    await this.deleteByLabel('AgentgatewayBackend', labels);
    ...
    // Also clean up any EnterpriseAgentgatewayPolicy created by this feature
    await this.deleteByLabel('EnterpriseAgentgatewayPolicy', labels);
  }
  ...
  async createBackend(provider) {
    ...
    this.log(`AgentgatewayBackend created for ${backendName}`, 'info');
  }
