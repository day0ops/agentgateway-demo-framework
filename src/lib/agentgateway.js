...
export class AgentGatewayManager {
  ...
  static async installCRDs(profileFile = null) {
    ...
    const { gatewayApiVersion, gatewayApiChannel, crdsVersion, crdsOciRegistry } =
  ...
  static async install(profileFile = null) {
    ...
    const { version, ociRegistry } = this.resolveVersionAndRegistry(profile);
  ...
  static async uninstall({ deleteNamespace = false } = {}) {
    ...
    try {
      // Check if helm release exists before attempting uninstall
      // Use -q flag to get just release names, one per line
      const result = await KubernetesHelper.helm(['list', '-n', AGENTGATEWAY_NAMESPACE, '-q'], {
        ignoreError: true,
      });

      // Check for exact release name match (not substring match)
